...
class userForm(tk.Tk):
    ...
    def fetch_employee_data(self, event=None):
        connection = sqlite3.connect("registration.db")
        cursor = connection.cursor()
        # Fetch the employee_id from the login credentials
        employee_id = self.login_employee_id  # Assuming you have a variable to store the employee_id from the login credentials
        
        cursor.execute("SELECT e.employee_id, e.name, e.residence, e.origin, e.email, e.dob, e.age, e.gender, e.contact, e.start_date, a.name, a.responsibilities, a.basic_salary, a.taxes, a.allowances, a.gross_pay, a.net_pay FROM employees e JOIN account a ON e.employee_id = a.employee_id WHERE e.employee_id=?", (employee_id,))
    ...
    def __init__(self):
        ...
        self.title("Pay Roll Systen/Employee Profile")
        ...
        self.heading_label = tk.Label(self.header_frame, bg="white", fg="royal blue", text="Employee PayRoll System >>  Employee Profile", font=("Times New Roman", 16, "bold"))
        ...
        self.delete_button = tk.Button(self.foot_frame, text="Print",  bg="gray", fg="white", font=("times new roman", 18), command=self.fetch_employee_details)
